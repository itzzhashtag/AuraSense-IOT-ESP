/*
  ...
  Also show appropriate data and Alert display for any Gas leakage from ESP3->ESP2->ESP1
...
// Line Number ->37,38,40,41,46,47,48,360,361,362          [Here www.tomorrow.io is being Used]
// Line Number -> 114 [Calibrate the values according to your Data Available]
...
// --- Matrix Pins ---
//===============================================
#define MAX_DEVICES 8                     // Number of Matrix display (8*8)
#define DATA_PIN   23                     // Data pin of Matrix Display
...
//===============================================
int temp = 0, hum = 0, aoq = 0, mq5Val = 0;          // Temperature Humidity AoQ Mq5 Values Stored here...
String weather_rp = "Loading weather...";            // Weather Report Stotred here
...
void setup()
{
  ...
  SPI.begin(CLK_PIN, -1, DATA_PIN);       // Initialize MAtrix Display Pins
  m1.begin(); m2.begin();                 // Start Matrix Display
  m1.setIntensity(1);                     // Set Brightness for Top Matrix
  m2.setIntensity(1);                     // Set Brightness for Bottom Matrix
  m1.displayClear();
  m2.displayClear();
  ...
  setupWebUI();                           // Setup the Web UI Interface UI/UX
  ...
  if (client)
  {
    ...
    while (client.connected())
    {
      if (client.available() >= sizeof(int) * 4)
      {
        ...
        client.read((uint8_t*)&lpg, sizeof(lpg));
        // === Garbage data filter ===
        bool valid = true;
        if (t < -40 || t > 100)
        {
        ...
        if (valid)
        {
          ...
          Serial.printf("✅ ESP1 received: Temp=%d, Hum=%d, AOQ=%d, LPG=%d\n", temp, hum, aoq, mq5Val);
        }
        else
        {
          temp = 0;
...
String weather_report()
{
  ...
  //You can try  https://api.tomorrow.io
  //You can try https://api.openweathermap.org
  //You can try https://api.weatherstack.com
  //You can try https://weather.visualcrossing.com
